% function [S] = poissonSolve(S, poisson_tol, Isguess, External_Electric_Field)
...
if S.cell_typ < 3
% 	[f,S.phi_extended] = poisson_RHS(S,External_Electric_Field);
	f = poisson_RHS(S);
else 
	f = -4*pi*(S.rho(:,1)+S.b);
end
...
% function [f,phi_extended] = poisson_RHS(S,External_Electric_Field)
...
%%%%%%%%% TESTING: store phi correction for later use %%%%%%%%%%%%%%%%%
% phi_extended = [];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     %%%%%%%%%%%% TESTING: we add linear term to V_Z_av %%%%%%%%%%%%%%%%%%%%
%     if ischar(External_Electric_Field)
%         if strcmp(External_Electric_Field,'OC')
%             % Add nothing
%         elseif strcmp(External_Electric_Field,'CC')
%             E_Z = -(V_Z_av(end)-V_Z_av(1))/LZ;   % Macroscopic electric field Z that naturally arises
%             V_Z_av = V_Z_av + (Z_bc(:)-mean(Z_bc)) * E_Z;     % We substract it to get zero macroscopic electric field
%         else
%             error('Wrong input: External_Electric_Field')
%         end
%     elseif isnumeric(External_Electric_Field)
%         V_Z_av = V_Z_av - (Z_bc(:)-mean(Z_bc)) * External_Electric_Field; % External electric field Z added on top
%    else
%        error('Wrong input: External_Electric_Field')
%     end
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
